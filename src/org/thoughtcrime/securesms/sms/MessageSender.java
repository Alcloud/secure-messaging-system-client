...
import org.thoughtcrime.securesms.AuthenticationHelper;
...
public class MessageSender {
  ...
  public static long send(final Activity activity,
  ...
  {
    String accountName = TextSecurePreferences.getAccountName(context);
    mAuthenticationHelper = new AuthenticationHelper(context, activity, accountName);
    ...
    sendTextMessage(activity, context, recipient, forceSms, keyExchange, messageId, message.getExpiresIn());
  ...
  public static void resend(Context context, MessageRecord messageRecord) {
    try {
      ...
      if (messageRecord.isMms()) {
        ...
        sendTextMessage(null, context, recipient, forceSms, keyExchange, messageId, expiresIn);
      }
    } catch (MmsException e) {
      Log.w(TAG, e);
    }
  ...
  private static void sendTextMessage(Activity activity, Context context, Recipient recipient,
                                      ...
                                      long messageId, long expiresIn)
  {
    ...
    if (!forceSms && isSelfSend(context, recipient)) {
      ...
      sendTextPush(activity, context, recipient, messageId);
    } else {
  ...
  private static void sendTextPush(Activity activity, Context context, Recipient recipient, long messageId) {
    JobManager jobManager = ApplicationContext.getInstance(context).getJobManager();
    jobManager.add(new PushTextSendJob(activity, context, messageId, recipient.getAddress()));
  }
