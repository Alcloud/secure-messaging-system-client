...
//    DecryptingQueue.schedulePendingDecrypts(context, masterSecret);
  ...
  public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
    ...
    if (oldVersion < NO_MORE_CANONICAL_ADDRESS_DATABASE) {
      ...
      NumberMigrator    numberMigrator                 = new NumberMigrator(TextSecurePreferences.getKVNR(context));
    ...
    if (oldVersion < NO_MORE_RECIPIENTS_PLURAL) {
      ...
      while (cursor != null && cursor.moveToNext()) {
        ...
        if (addressList.length == 1) {
          ...
          members.add(DelimiterUtil.escape(TextSecurePreferences.getKVNR(context), ','));
    ...
    if (oldVersion < INTERNAL_DIRECTORY) {
      ...
      while (cursor != null && cursor.moveToNext()) {
        String        address       = new NumberMigrator(TextSecurePreferences.getKVNR(context)).migrate(cursor.getString(0));
        ContentValues contentValues = new ContentValues(1);
